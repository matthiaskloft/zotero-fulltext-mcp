...
| 6 | Package 5 step 4: adversarial and containment tests | hardening plan | none | READY | Tampered `current.json` generation identifiers, path escapes on MCP reads and maintenance writes, lock-ownership races, untrusted instruction text in titles and snippets. `resolve_generation_dir` already validates and contains; this proves it against hostile input. Covers shipped code, so nothing gates it. |
| 7 | Package 5 step 2: schema-compatibility tests | hardening plan | none | READY | An index written by an older version must migrate through a documented command or fail with a precise recovery instruction, never a raw SQLite error. Covers Package 2, which shipped at rank 4. |
...
Ranks 1-5 are done. Start at **rank 6** (adversarial and containment tests): it covers already
shipped code, needs no new subsystem, and nothing gates it.
