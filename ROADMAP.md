...
Ranks 1-6 are done. Start at **rank 7** (schema-compatibility tests): like rank 6 it covers already
shipped code — an index written by an older version must migrate through a documented command or
fail with a precise recovery instruction, never a raw SQLite error.
