...
9. **TODO — `config.<hostname>.json` uses `platform.node()` (the hostname), not the username.**
   ...
   box where the hostname (`WORKSTATION-01`) differs from the username (`jsmith`), a
   `config.jsmith.json` never auto-resolves and the registration has to bake in an explicit
